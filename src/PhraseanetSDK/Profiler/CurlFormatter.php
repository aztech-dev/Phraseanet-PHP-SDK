 ...
 * @link https://github.com/namshi/cuzzle
 */
class CurlFormatter
{
    /**
    ...
    protected function extractBodyArgument(RequestInterface $request)
    {
        if ($request instanceof EntityEnclosingRequest) {
            if ($request->getBody()) {
                $this->addOption('d', escapeshellarg($request->getBody()));
            }

            if ($request->getPostFields()) {
                $this->addOption('d', escapeshellarg((string)$request->getPostFields()));
            }
        }
    ...
    protected function extractHeadersArgument(RequestInterface $request)
    {
        ...
        foreach ($request->getHeaders()->getAll() as $name => $header) {
